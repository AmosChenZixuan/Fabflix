...
function submitFulltextSearch(event){
    let query = $("#fulltext-bar").val();
    console.log("You entered fulltext search query: " + query);
    event.preventDefault();
    $.ajax("api/main-page", {
        method:"POST",
        data:{'action':'fulltext', 'query':query},
    });
    //}
...
function handelMovieList(resultData){
    ...
    if (resultData['fulltext_query'] != null)
        $("#fulltext-bar").val(resultData["fulltext_query"]);
